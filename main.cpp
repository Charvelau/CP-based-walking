...
int main(){
    ...
    MatrixXd cp_knot=cpdemo.cp_eos_backward(zmp_control_point,omega,3);
    //cout<<cp_knot<<endl;
    MatrixXd cp_traj=cpdemo.cp_tracking_forward(zmp_control_point,omega,cp_knot);
    //cout<<cpdemo.cp_tracking_forward(zmp_control_point,0.3,cp_knot)<<endl;
    MatrixXd zmp_trajec=cpdemo.zmp_trajec(cp_traj,cp_knot,omega);
